...
7. Create at least one application user through `/login` so the profile trigger runs.
8. `002_seed.sql` optionally loads the fictional IST Data Center Expansion dataset and attributes it to the earliest profile. Skip this file when preserving a real project dataset.
