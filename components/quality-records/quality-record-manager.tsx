...
export function QualityRecordManager({ module, title, description, noun, route, rows, columns, fields, createAction, updateAction, deleteAction, loadError, createDefaults }:
  ...
  function save(formData: FormData) { if (editing?.id) formData.set('id', editing.id); setFeedback(null); startTransition(async () => { let result; try { result = editing?.id ? await updateAction(formData) : await createAction(formData); } catch { result = { success: false, message: 'Save failed. Check your connection and try again.' }; } setFeedback(result); if (result.success) { setEditing(undefined); router.refresh(); } }); }
  function remove() { if (!deleting) return; const formData = new FormData(); formData.set('id', deleting.id); startTransition(async () => { let result; try { result = await deleteAction(formData); } catch { result = { success: false, message: 'Delete failed. Check your connection and try again.' }; } setFeedback(result); if (result.success) { setDeleting(null); router.refresh(); } }); }
  const openCreate = () => { setEditing(null); setFeedback(null); };

  if (!fields.find((f) => f.name === 'project_id')?.options?.length) return <ProjectRequired message="The active project has no EAS-6-B mapping. Select a project using IL051 (or its IL05 alias) to create these records." />;
  ...
  return <div className="mx-auto flex w-full max-w-[1680px] flex-col gap-5">
    ...
    <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5" aria-label={`${module} summary`}>
      <Metric label="Total" value={counts.total} /><Metric label="In review" value={counts.review} tone="violet" /><Metric label={module === 'RRR' ? 'Verified / Accepted / Released' : ['NCR', 'SOR'].includes(module) ? 'Verified / Closed' : 'Approved / Proceed'} value={counts.approved} tone="green" /><Metric label="Rejected" value={counts.rejected} tone="red" /><Metric label="Open / Action required" value={counts.open} tone="amber" />
    </section>
