...
export default async function SorDetailPage({ params }: { params: Promise<{ id: string }> }) { const { id } = await params; const client = await createClient(); const [record, actions] = await Promise.all([getV2Record(client, 'sor_records', id), listQualityActions(client, 'SOR', id)]); if (record.error) throw new Error(record.error); if (!record.data) notFound(); const r = record.data;
  ...
  ]}>{actions.error ? <p role="alert" className="text-sm text-red-700">{actions.error}</p> : null}<ActionTracker rows={actions.data} projectId={r.project_id} parentType="SOR" parentId={r.id} addAction={addQualityAction} /><RecordLinks type="SOR" id={id} /></QualityRecordDetail>; }
