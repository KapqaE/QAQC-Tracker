...
export async function listNcrs(client: SupabaseClient<Database>): Promise<ServiceResult<Tables<'ncrs'>[]>> { const { data, error } = await client.from('ncrs').select('*').order('due_date', { ascending: true }); return { data: data ?? [], error: error ? serviceError(error, 'NCRs could not be loaded.') : null }; }
